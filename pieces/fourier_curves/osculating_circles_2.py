...
def make_frame(t):
    ...
    p = interval_progresses(progress, 6, 'none')

    # Fourier series coefficients
    harmonics = [
            (8,  1*(p[0] - p[4]) + 1.0, np.pi/2),
            (16, 1*(p[1] - p[3]) + 0.5, np.pi/2),
            (24, 1*(p[1] - p[3]) + 0.5, np.pi/2),
            (32, 1*(p[1] - p[5]) + 0.0, np.pi/2),
            (40, 1*(p[2] - p[5]) + 0.5, np.pi/2),
            (48, 1*(p[2] - p[4]) + 0.5, np.pi/2),
            (4,  1*(p[3] - p[5]) + 0.0, np.pi/2)
        ]
    n_points = 27000
